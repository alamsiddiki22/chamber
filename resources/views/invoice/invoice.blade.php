			...
			<table>
				<tr class="top">
					<td colspan="4">
						<table>
							<tr>
								<td class="title">
									<img width="200" src="https://i.postimg.cc/nhrG5gL0/download.png" alt="Company logo"/>
								</td>

								<td>
									Invoice #: {{ $order_id }}<br />
									{{ App\Models\Order::where('id', $order_id)->first()->created_at->format('d-M-Y') }}<br />
								</td>
							</tr>
						</table>
					</td>
                ...
                @foreach (App\Models\OrderProduct::where('order_id', $order_id)->get() as $order)
                    <tr class="item">
                        ...
                        <td style="text-align: right">{{ $order->price * $order->quantity}}</td>
                    </tr>
                    @php
				...
				<tr class="total" style="text-align: right">
					<td colspan="3"></td>
					<td>Discount: {{ App\Models\Order::where('id', $order_id)->first()->discount }}</td>
				</tr>
				<tr class="total" style="text-align: right">
					<td colspan="3"></td>
					<td>charge: {{ App\Models\Order::where('id', $order_id)->first()->charge }}</td>
				</tr>
				<tr class="total" style="text-align: right">
					<td colspan="3"></td>
					<td>Grand Total: {{ App\Models\Order::where('id', $order_id)->first()->total }}</td>
				</tr>
			</table>
		</div>
	</body>
